...
if DST and GLOBAL.TheNet:IsDedicated() then
    return
end

local GetPlayer = DST and function()
    return GLOBAL.ThePlayer
end or GLOBAL.GetPlayer
...
local function GetConfig(configname, default, validator)
    if type(validator) == "string" then
        ...
        validator = function(value)
            return type(value) == validator_type
        end
    end
...
-- 一些特殊的物品的放置偏移，包括墙体，tile和flood tile
...
local HIDECURSOR = GetConfig("HIDECURSOR", false, function(val)
    return type(val) == "boolean" or val == 1
end)
...
function Placer:SetCursorVisibility(show)
    ...
    if ThePlayer and ThePlayer.HUD and ThePlayer.HUD.controls  -- HUD.controls 玩家HUD控件的各种信息
            and ThePlayer.HUD.controls.mousefollow and ThePlayer.HUD.controls.mousefollow.children then -- mousefollow 鼠标控件
        ...
        if cursor_object then
            ...
            if show then
                if cursor_object.image then
                    cursor_object.image:Show()
                end
                if cursor_object.quantity then
                    cursor_object.quantity:Show()
                end
                if cursor_object.percent then
                    cursor_object.percent:Show()
                end
            else
                if cursor_object.image then
                    cursor_object.image:Hide()
                end
                if cursor_object.quantity and HIDECURSORQUANTITY then
                    cursor_object.quantity:Hide()
                end
                if cursor_object.percent then
                    cursor_object.percent:Hide()
                end
            end
        end
    end
...
function Placer:MakeGridInst()
    ...
    gridinst.Transform:SetRotation(self.geometry.gridplacer_rotation + 45) -- 网格旋转45度默认
    gridinst.Transform:SetScale(1.7, 1.7, 1.7) -- 放大比例1.7倍
    return gridinst
end
...
function Placer:RefreshBuildGrid(time_remaining)
    --if not time_remaining, then config was set to no limit
    if time_remaining then
        if time_remaining < 0 then
            return
        end --we were over time already (common on generation updates)
...
function Placer:OnUpdate(dt)
    ...
    elseif not TheInput:ControllerAttached() then
        ...
        elseif self.snap_to_meters then
            ...
            --#rezecib Added this block, everything else should match Placer:OnUpdate
            pt = Snap(pt)
        end
    else
        -- Controller input
    ...
    if self.tileinst then
        self.tileinst.Transform:SetPosition(Vector3(TheWorld.Map:GetTileCenterPoint(pt:Get())):Get())
    end
    ...
    if pt and pt.x and pt.z and not (hadgrid and lastpt and lastpt.x == pt.x and lastpt.z == pt.z) then
        ...
        if hadgrid then
            --We can just move the existing grid, or some of its points
            ...
            if self.geometry.HasOverlap(dx, dz, GRID_SIZE) then
                ...
                for bgx = lx + start_row, lx + end_row do
                    ...
                    if cx + start_row <= bgx and bgx <= cx + end_row then
                        --if this row appears in both
                        ...
                        -- this is an old row, we can remove all of its points
                ...
                for bgx = cx + start_row, cx + end_row do
                    ...
                    local start_col_new, end_col_new = unpack(self.rowbounds[bgx - cx])
                    if lx + start_row <= bgx and bgx <= lx + end_row then
                        --if this row appears in both
                        ...
                        -- this is an new row, we will add all of its points
                        self.build_grid[bgx] = {}
    ...
    if #self.refresh_queue == 0 then
        --We have nothing left to refresh, queue the whole grid again
        for bgx, row in pairs(self.build_grid) do
            for bgz, bgp in pairs(row) do
                table.insert(self.refresh_queue, { bgx, bgz })
            end
...
local function sprinklerPlaceTestFn(inst, pt)
    ...
    if not valid_sprinkler_tile then
        return false
    end
...
local function ReplaceGridplacerAnim(inst)
    if inst._geo_replaced_anim then
        return
    end
...
local function UndoReplaceGridplacerAnim(inst)
    if not inst._geo_replaced_anim then
        return
    end
...
local function PlacerPostInit(self)
    ...
    self.inst:ListenForEvent("onremove", function()
        ...
        if self.gridinst then
            self.gridinst:Remove()
        end
        if self.controller_child then
            self.controller_child:Remove()
        end
    end)
...
if DST then
    ...
    for action, _ in pairs(RMB_ACTION_GRID_SPACING) do
        ACTIONS_TO_SNAP[action] = function()
            return ACTION_ENABLED[action]
        end
    end
    local ACTION_CODES_TO_SNAP = {}
    for action, _ in pairs(ACTIONS_TO_SNAP) do
        if rawget(GLOBAL.ACTIONS, action) then
            ACTION_CODES_TO_SNAP[GLOBAL.ACTIONS[action].code] = function()
                return ACTIONS_TO_SNAP[action]()
            end
        end
    end
...
local function PushOptionsScreen()
    ...
    screen.callbacks.save = function()
        ...
        end --janky, but KnownModIndex functions kinda spam the logs
        ...
        GLOBAL.KnownModIndex:SaveConfigurationOptions(function()
    ...
    local function update_color_spinners_buttons()
        ...
        if preset then
            if DST then
                screen.color_buttons[preset]:Select()
            else
                screen.color_buttons[preset]:Disable()
            end
        end
    end
    ...
    if CTRL then
        screen.toggle_button.onclick()
    end
    screen.callbacks.toggle = function(toggle)
        CTRL = not toggle
    end
    if not BUILDGRID then
        screen.grid_button.onclick()
    end
    ...
    if HIDEPLACER then
        screen.placer_button.onclick()
    end
    screen.callbacks.placer = function(toggle)
        HIDEPLACER = toggle == 0
    end
    if HIDECURSOR then
        screen.cursor_button.onclick()
    end
    if HIDECURSORQUANTITY then
        screen.cursor_button.onclick()
    end
    ...
    if not SMARTSPACING then
        screen.smart_spacing_button.onclick()
    end
    ...
    if screen.till_grid_button then
        if not ACTION_ENABLED.TILL then
            screen.till_grid_button.onclick()
        end
